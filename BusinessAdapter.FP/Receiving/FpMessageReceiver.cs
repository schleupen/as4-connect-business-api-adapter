        ...
        : IFpMessageReceiver
    {
            ...
            ReceiveStatus receiveStatus)
        {
            ...
            for (var i = 0; i < messageLimit; i++)
            {
                ...
                {
                    receiveStatus.AddFailedReceivedMessage(currentMessage, fileAlreadyExistException);
                    exceptions.Add(fileAlreadyExistException);
                }
                catch (Exception ex)
                {
                    receiveStatus.AddFailedReceivedMessage(currentMessage, ex);
                    messagesForRetry.Add(currentMessage);
                    exceptions.Add(new RetryableException(
                        $"Error while receiving message with identification '{currentMessage.MessageId}'. [{currentMessage.BDEWProperties}]",
                        ex));
                }
            }
            ...
            ReceiveStatus receiveStatus)
        {
            ...
            if (!ackResponse.WasSuccessful)
            {
                ...
                throw new InvalidOperationException(
                    $"Error while acknowledging messages with id '{message.MessageId}'.");
            }
